...
long call(pid_t pid, char *lib_string, void *local_function, int nargs, ...) {
  ...
  **/
  ...
  int32_t fprintf_delta = compute_jmp(mmap_memory, their_func);
  ...
  // for now we are just using this for dup2 and pipe, which each have two arguments.  
  // will look at extending support for nargs later
 	va_list vl;
	va_start(vl,nargs);
  int i = 0;
